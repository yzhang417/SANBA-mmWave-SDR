function [x1,estFin,optFin,estHist] = VampGlmEst(denoiser,likelihood,A,opt)
...
% USAGE
% -----
% [x1,estFin,optFin,estHist] = VampGlmEst(denoiser,likelihood,A,[opt])
%
...
dC = opt.dampConfig; % chooses which quantities to damp
verbose = opt.verbose; % verbose output?
silent = opt.silent; % disable all warnings?
debug = opt.debug; % disable debugging?
altUpdate = opt.altUpdate; % alternating update of x and z, or simultaneous?
...
elseif any(strcmp(superclasses(likelihood),'EstimOut')) 
    ...
    error('The likelihood must be either an EstimOut object, a fxn handle that accepts [phat,pvar] and produces [zhat,zvar], or a fxn handle that accepts [rhat,rvar] and produces only xhat.')
end 
...
if isa(A,'numeric')
    ...
    if Mtest~=M, 
        error('Number of rows in y and A do not match'); 
    end;
...
elseif isa(A,'function_handle')||any(strcmp(superclasses(A),'LinTrans'))
    ...
    if isa(A,'function_handle')
        ...
        if ~isempty(opt.N)
            ...
            error('Since A is a fxn handle, must specify opt.N');  
        end
    ...
    if debug
      try
        z = fxnA(zeros(N,L));
        if any(size(z)~=[M,L]), 
            error('output size of A doesnt match that of likelihood');
        end
        clear z;
      catch
        error('fxnhandle A doesnt accept inputs of correct size')
      end
    end
    ...
    if debug
      try
        x = fxnAh(zeros(M,L));
        if any(size(x)~=[N,L]), 
            error('output size of opt.Ah doesnt match that of likelihood');
        end
        clear x;
      catch
    ...
    error('A must be a matrix, a LinTrans object, or a fxnhandle to a linear operator')
end
del = M/N;
...
if ~isempty(opt.r1init)
    r1 = opt.r1init;
    if debug
        if ~all(size(r1)==[N,L]), error('need size(opt.r1init)==[N,L]'); end;
    end;
else
...
if ~isempty(opt.p1init)
    p1 = opt.p1init;    
    if debug
        if ~all(size(p1)==[M,L]), error('need size(opt.p1init)==[M,L]'); end;
    end;
else
...
% Initialize variables needed for warm-start and damping 
r1old = opt.r1old;  
r2old = opt.r2old; 
p1old = opt.p1old; 
p2old = opt.p2old; 
...
z1old = opt.z1old; 
z2old = opt.z2old; 
gam1xold = opt.gam1xold;  
gam1zold = opt.gam1zold; 
gam2xold = opt.gam2xold; 
gam2zold = opt.gam2zold; 

% Initialize other stuff
if altUpdate 
  % these won't be computed during first iteration, but are needed
  gam2x = gam1x; 
  r2 = r1; 
  x1 = r1; 
  eta1x = nan(1,L); 
...
while ~stop
  ...
  if ~altUpdate || mod(i,2) % update on odd i
    ...
    if dC(1) % apply damping
      if ~isempty(z1old), z1 = damp*z1 + (1-damp)*z1old; end
    end
    gam2z = eta1z - gam1z; 
    p2 = bsxfun(@rdivide,bsxfun(@times,z1,eta1z)-bsxfun(@times,p1,gam1z),gam2z);
    % NOTE: p2=z+N(0,1/gam2z)
    if ~silent
      if any(gam2z<gamMin), 
  ...
  if dC(2) % apply damping
    if ~isempty(gam2zold), gam2z = dampGam*gam2z + (1-dampGam)*gam2zold; end
  end
  if dC(3) % apply damping 
    if ~isempty(p2old), p2 = damp*p2 + (1-damp)*p2old; end
  end

  if ~altUpdate || ~mod(i,2) % update on even i
    [x1,xvar1] = fxnDenoise(r1,ones(N,1)*(1./gam1x));
    eta1x = 1./mean(xvar1,1); 
    if dC(4) % apply damping
      if ~isempty(x1old), x1 = damp*x1 + (1-damp)*x1old; end
    end
    gam2x = eta1x - gam1x; 
    r2 = bsxfun(@rdivide,bsxfun(@times,x1,eta1x)-bsxfun(@times,r1,gam1x),gam2x);
    % NOTE: x=r2+N(0,1/gam2x)
    if ~silent
      if any(gam2x<gamMin), 
    ...
    if dC(5) % apply damping
      if ~isempty(gam2xold), gam2x = dampGam*gam2x + (1-dampGam)*gam2xold; end
    end
    if dC(6) % apply damping
      if ~isempty(r2old), r2 = damp*r2 + (1-damp)*r2old; end
    end
  end
  ...
  if dC(7) % apply damping
    if ~isempty(z2old), z2 = damp*z2 + (1-damp)*z2old; end
  end
  if dC(10) % apply damping
    if ~isempty(x2old), x2 = damp*x2 + (1-damp)*x2old; end
  end
  ...
  if ~altUpdate || mod(i,2) % update on odd i
    ...
    if dC(11)
      if ~isempty(gam1xold), gam1x = dampGam*gam1x + (1-dampGam)*gam1xold; end
    end
    if dC(12)
      if ~isempty(r1old), r1 = damp*r1 + (1-damp)*r1old; end
    end
  end
  ...
  if dC(8) 
    if ~isempty(gam1zold), gam1z = dampGam*gam1z + (1-dampGam)*gam1zold; end
  end
  if dC(9) 
    if ~isempty(p1old), p1 = damp*p1 + (1-damp)*p1old; end
  end
  ...
  if (~isempty(r1old))&&(~isempty(p1old))
      change = max([sum(abs(r1-r1old).^2)./sum(abs(r1).^2),...
                sum(abs(p1-p1old).^2)./sum(abs(p1).^2)]);
  else
      change = inf;
  end
  if (change < tol^2)||(i>=nitMax)
      stop = true;
  else
      r1old = r1;
      r2old = r2;
...
estFin.r1old = r1old; % for warm-start with damping
estFin.r2old = r2old; % for warm-start with damping
estFin.p1old = p1old; % for warm-start with damping
estFin.p2old = p2old; % for warm-start with damping
estFin.x1old = x1old; % for warm-start with damping
estFin.x2old = x2old; % for warm-start with damping
estFin.z1old = z1old; % for warm-start with damping
estFin.z2old = z2old; % for warm-start with damping
estFin.gam1xold = gam1xold; % for warm-start with damping
estFin.gam1zold = gam1zold; % for warm-start with damping
estFin.gam2xold = gam2xold; % for warm-start with damping
estFin.gam2zold = gam2zold; % for warm-start with damping
estFin.fxnDenoise = fxnDenoise; % for warm-start 
